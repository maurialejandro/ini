...
const CamposView = () => {
  ...
  const handleCreate = () => router.replace(`/proveedores/crear/`);
...
const TableCampos = forwardRef((props: DataViewProps, ref) => {
  ...
  const [targetDelete, setTargetDelete] = useState<Proveedor | null>(null);
  // const user = userAuth;
  ...
  const { loadProviders, loadMoreProviders, loadCount, loadExportData } =
    useProveedoresList({
      filtro,
    });

  const {
    get: { data: proveedores, isLoading, count },
    handleReload,
    handleLoadData,
    handleLoadMoreData,
  } = newUseFirebaseData({
    filter: filtro,
    getData: loadProviders,
    getDocsCount: loadCount,
    getDocsCountFiltered: loadCount,
    getMoreData: loadMoreProviders,
  });
  ...
  const handleConfirmDialog = async () => {
    ...
    const res = await deleteProvider(targetDelete);
    if (res === "OK") {
  ...
  const handleGoToSelected = (selected: any) => {
    router.replace(`/proveedores/ver/${selected.id}`);
  };
  ...
  return (
    <>
                ...
                <TableCell padding="checkbox">
                  <Checkbox
                    indeterminate={
                      ...
                      rowsSelected.length < proveedores.length
                    }
                    checked={
      ...
      <ConfirmDialog
        ...
        textContent="Se eliminará este proveedor. Esta acción no se puede deshacer"
